...
class Product:
    def __init__(self,name,price,category):
        if price < 0:
            raise ValueError("Price cannot negative")
...
class Customer:
    tier = "none"           # base tier; then Silver / Gold / Platinum subclasses
...
class Order:              # has-a Customer, has-many OrderItem
    ...
    def subtotal(self):
        ...
        return total        #     def subtotal(self): ...     # pure: returns a number
    
    def discount(self):
        subtotal = self.subtotal()
        discount_amount = subtotal * self.customer.discount(subtotal)
        ...
        return discount_amount      #     def discount(self): ...     # pure
    ...
    def tax(self):
        ...
        for item in self.items:
            if item.product.category != FOOD_CATEGORY:
                tax_amount += item.product.price * item.quantity * TAX_RATE

        return tax_amount#     def tax(self): ...          # pure
    
    def total(self):
        return self.subtotal() - self.discount() + self.tax()#     def total(self): ...        # pure
    
    def points(self):
        return int(self.total() // POINTS_DIVISOR) * self.customer.multiplier()#     def points(self): ...       # pure

    def receipt(self):
        print("Receipt for " + self.customer.name + " (" + self.customer.tier + ")")
        print("-" * 40)

        for item in self.items:
            line_total = item.product.price * item.quantity
            print(item.product.name + " x" + str(item.quantity) + " = " + str(line_total))
        ...
        print("")#     def receipt(self):          # builds the receipt text (no calculation here)
...
def refactored_main():
        ...
        orders = [
            Order(alice, alice_items),
            Order(bob, bob_items),
            Order(charlie, charlie_items),
            Order(dana, dana_items),
        ]
